-- ---------------------------------------------------------------------------
-- 020_fix_slot_locking.sql
-- Simplified, correct slot-locking rules:
--
--   1) Any CONFIRMED appointment blocks the slot for everyone.
--   2) Any PAID card payment also blocks the slot.
--   3) A PENDING card payment holds the slot for 30 minutes (checkout window).
--      After 30 minutes the slot is released automatically (user abandoned).
--
-- The previous p_request_is_prepaid distinction (cash-reserved slots visible
-- to prepaid customers) caused confusion and left slots unblocked after
-- payment success. Removing it for simplicity — a booked slot is a booked slot.
-- ---------------------------------------------------------------------------
...
slot_series AS (
  ...
  FROM staff_hours sh,
    generate_series(
      ...
      (
        p_date::text || ' ' || sh.wh_end::text || ' +00'
      )::timestamptz - (v_duration || ' minutes')::interval,
      (v_slot_interval || ' minutes')::interval
    ) gs
),
booked AS (
  -- A slot is blocked when ANY of the following is true:
  --   • The appointment has been confirmed (covers both cash and paid card).
  --   • The payment has been marked paid/succeeded by the webhook.
  --   • A pending card payment exists and was created within the last
  --     30 minutes (active checkout session — prevents double-booking).
