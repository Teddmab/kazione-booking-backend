-- ─────────────────────────────────────────────────────────────────────────────
-- 145_owner_availability_seat_counts.sql
--
-- 144 let an owner see a busy/over-capacity slot instead of hiding it, but
-- only ever exposed a boolean at_capacity — "full" vs "not full", nothing
-- in between. Feedback after seeing it live: a blanket "busy" treatment
-- doesn't tell the owner whether one seat is taken and two remain free, or
-- whether the salon is genuinely full — and the same is true for staff
-- (already derivable client-side from the per-staff has_staff_conflict
-- rows this function already returns, no backend change needed for that
-- half).
--
-- Replaces at_capacity (boolean) with seats_free / seats_total (int,
-- both NULL when this business doesn't track seat capacity at all — no
-- seats concept to report). seats_free = how many MORE bookings could
-- still fit at this instant, not counting the one being considered.
-- ─────────────────────────────────────────────────────────────────────────────

DROP FUNCTION IF EXISTS get_available_slots(uuid, uuid, uuid, date, int, boolean);
...
BEGIN
  ...
  -- staff) row, as plain values — filtering happens one CTE layer down so
  -- p_owner_override can choose which flags actually exclude a row instead
  -- of duplicating each subquery once per branch.
  slot_flags AS (
    SELECT ss.s_instant, ss.sp_id, ss.display_name, ss.eff_price,
           ...
           CASE WHEN v_capacity_enforced THEN (
             SELECT COUNT(*)
               FROM appointments a
              WHERE a.business_id = p_business_id
  ...
  free_slots AS (
    SELECT sf.s_instant, sf.sp_id, sf.display_name, sf.eff_price,
           sf.is_staff_conflict,
           CASE WHEN v_capacity_enforced
                THEN GREATEST(v_seat_count - sf.overlapping_count, 0)::int
           END AS seats_free_calc
      FROM slot_flags sf
     WHERE NOT sf.is_time_off
       AND (p_owner_override OR NOT sf.is_staff_conflict)
       AND (p_owner_override OR NOT v_capacity_enforced OR sf.overlapping_count < v_seat_count)
  ),
  slot_counts AS (
  ...
  SELECT DISTINCT
         ...
         fs.seats_free_calc   AS seats_free,
         (CASE WHEN v_capacity_enforced THEN v_seat_count END) AS seats_total
    FROM free_slots fs
