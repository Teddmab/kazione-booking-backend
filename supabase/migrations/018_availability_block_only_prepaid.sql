-- ---------------------------------------------------------------------------
-- 018_availability_block_only_prepaid.sql
-- Availability should block only prepaid bookings (card/deposit/full),
-- while pay-later (cash) reservations remain visible/available.
-- ---------------------------------------------------------------------------
...
booked AS (
  ...
  WHERE a.business_id = p_business_id
    ...
    AND (
      -- Keep pay-later reservations (cash method) visible in availability.
      p.id IS NULL
      OR p.method <> 'cash'
    )
),
